...
class HierarchicalTextSplitter:
    ...
    def _is_technical_content(self, text: str) -> bool:
        ...
        technical_indicators = [
            'table', 'figure', 'equation', 'section', 'Â§', 
...
def validate_model_config(model_config: Dict[str, Any]) -> Tuple[bool, List[str]]:
    """Validate model configuration parameters - UPDATED for OpenAI/Google only."""
    errors = []
    ...
    if provider not in ["OpenAI", "Google"]:
        errors.append(f"Unsupported provider: {provider}. Only OpenAI and Google are supported.")
        return False, errors
...
def get_models(model_config: Dict[str, Any]) -> Tuple[Optional[Any], Optional[Any]]:
    """Initialize LLM and embedding models - UPDATED for OpenAI/Google only."""
